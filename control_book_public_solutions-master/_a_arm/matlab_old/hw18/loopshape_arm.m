...
% increase tracking by factor of 10 below omega_r=0.007
  ...
  plot(w,20*log10(1/gamma_r)*ones(1,length(Pmag_))...
         +20*log10(Pmag_),'g')    

% attenuate noise by factor of gamma_n above omega_n
  omega_n = 1000;  % attenuate noise above this frequency
  gamma_n = 0.1;    % amount of noise attenuation in output
  w = logspace(log10(omega_n),log10(omega_n)+2);
  Pmag=bode(Plant*C_pid,w);
  for i=1:size(Pmag,3), Pmag_(i)=Pmag(1,1,i); end
  plot(w,20*log10(gamma_n)*ones(1,length(Pmag_))...
         +20*log10(Pmag_),'g')
  figure(3), margin(Plant*C_pid)
