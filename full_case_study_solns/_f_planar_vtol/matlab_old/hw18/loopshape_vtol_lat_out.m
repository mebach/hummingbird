...
Plant = P_lat_out*P_lat_in*C_lat_in/(1+P_lat_in*C_lat_in);
figure(7), clf
...
%         plot(w,20*log10(1/gamma_din)*ones(1,length(Pmag_))+20*log10(Pmag_),'g')
        ...
        gamma_n = 10^(-100/20);   % attenuate noise by this amount
...
%         plot(w,20*log10(1/gamma_2)-40*log10(w),'g')
  ...
  figure(7), bode(Plant,logspace(-3,5)), grid on
  %print('../../../figures/hw_vtol_lat_out_compensator_design_1','-dpdf','-bestfit')
...
% proportional control: change cross over frequency
     kp = -0.03;
     C = C*kp;
     figure(7), margin(Plant*C)
     %print('../../../figures/hw_vtol_lat_out_compensator_design_2','-dpdf','-bestfit')
     
% phase lead: increase PM (stability)
    wmax = 1.3; % location of maximum frequency bump
    M    = 45; % separation between zero and pole
    Lead =tf(M*[1,wmax/sqrt(M)],[1,wmax*sqrt(M)]);
    C = C*Lead;
    figure(7), margin(Plant*C),  % update plot
     %print('../../../figures/hw_vtol_lat_out_compensator_design_3','-dpdf','-bestfit')
...
% integral control: increase steady state tracking and dist rejection
     ...
     C = C*Integrator;
     figure(7), margin(Plant*C),  % update plot
     %print('../../../figures/hw_vtol_lat_out_compensator_design_4','-dpdf','-bestfit')
...
% low pass filter: decrease gain at high frequency (noise)
     ...
     C = C*LPF;
     figure(7), margin(Plant*C),  % update plot
     %print('../../../figures/hw_vtol_lat_out_compensator_design_5','-dpdf','-bestfit')
...
% low pass filter
    p = .5;  % frequency to start the LPF
    LPF = tf(p, [1,p]);
    F = F*LPF;
...
figure(8), clf
    subplot(3,1,1), 
        ...
        bodemag(P_lat_in*C_lat_in/(1+P_lat_in*C_lat_in))
    ...
    subplot(3,1,3), 
     ...
     %print('../../../figures/hw_vtol_lat_out_compensator_design_6','-dpdf','-bestfit')
       
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Convert controller to state space equations for implementation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[num,den] = tfdata(C,'v');
[P.A_lat_out_C,P.B_lat_out_C,P.C_lat_out_C,P.D_lat_out_C]=tf2ss(num,den);

[num,den] = tfdata(F,'v');
[P.A_lat_out_F, P.B_lat_out_F, P.C_lat_out_F, P.D_lat_out_F] = tf2ss(num,den);
