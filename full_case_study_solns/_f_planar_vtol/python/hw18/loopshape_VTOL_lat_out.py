...
Plant = tf.minreal(P_lat_out*(L_in.P_lat_in*L_in.C/(1+L_in.P_lat_in*L_in.C)))

PLOT = False
# PLOT = True

if PLOT: plt.figure(7), plt.clf()
...
#----------- noise specification --------
omega_n = 10**(2.0)    # attenuate noise above this frequency
gamma_n = 10**(-100.0/20.0)    # attenuate noise by this amount
...
w_max = 1.3  #location of maximum frequency bump (desired crossover)
M = 45.0
...
mag, phase, omega = bode(OPEN, dB=True, omega=np.logspace(-3, 5), Plot=False)
if PLOT: 
    plt.subplot(211)
...
if PLOT:
    ...
    mag, phase, omega = bode(CLOSED_R_to_Y*F, dB=True, Plot=False)
