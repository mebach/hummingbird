...
class VTOLController:
    ...
    def u(self, r, y):
        ...
        F_tilde = -P14.K_lon*self.xhat_lon - P14.ki_lon*self.integrator_h
        F = P.Fe/np.cos(theta_hat) + F_tilde - self.dhat_lon
        tau = -P14.K_lat*self.xhat_lat - P14.ki_lat*self.integrator_z - self.dhat_lat
    ...
    def updateObserver(self, y_m):
        ...
        for i in range(0, N):
            self.xhat_lon = self.xhat_lon + P.Ts/float(N)*(
                P14.A_lon*self.xhat_lon
                + P14.B_lon*(self.F_d1 - P.Fe + self.dhat_lon )
                + P14.L_lon*(y_lon-P14.C_lon*self.xhat_lon))
            self.dhat_lon = self.dhat_lon + P.Ts/float(N)*(
                P14.Ld_lon*(y_lon-P14.C_lon*self.xhat_lon))
            self.xhat_lat = self.xhat_lat + P.Ts/float(N)*(
                P14.A_lat*self.xhat_lat
                + P14.B_lat*(self.tau_d1 + self.dhat_lat)
                + P14.L_lat*(y_lat-P14.C_lat*self.xhat_lat))
            self.dhat_lat = self.dhat_lat + P.Ts/float(N)*(
                P14.Ld_lat*(y_lat-P14.C_lat*self.xhat_lat))
