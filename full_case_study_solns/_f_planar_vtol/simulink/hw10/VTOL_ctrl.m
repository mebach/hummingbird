function u=VTOL_ctrl(in,P)
    ...
    % equilibrium force
    F_tilde = PID_h(h_d,h,flag,P.kp_h,P.ki_h,P.kd_h,P.Ftildemax,P.Ts,P.sigma);
    F = P.Fe + F_tilde;
    
    % lateral control for position
    % outer-loop: compute desired pitch angle using position error
    theta_d = PID_z(z_d,z,flag,P.kp_z,P.ki_z,P.kd_z,30*pi/180,P.Ts,P.sigma);
    % inner-loop: compute the torque using pitch error
    tau = PD_th(theta_d,theta,flag,P.kp_th,P.kd_th,P.taumax,P.Ts,P.sigma);
...
function u = PID_h(h_d,h,flag,kp,ki,kd,limit,Ts,sigma)
